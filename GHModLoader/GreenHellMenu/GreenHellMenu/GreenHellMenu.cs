...
public class GreenHellMenu : Mod
{
    ...
    public void Start()
    {
        ...
        AssetBundleCreateRequest request = AssetBundle.LoadFromMemoryAsync(GetEmbeddedFileBytes("greenhell.assets"));
        ...
        {
            //var xAxis = menu.transform.Find("Background").Find("TeleTab").Find("InputFieldX").GetComponent<InputField>().text;
            //var yAxis = menu.transform.Find("Background").Find("TeleTab").Find("InputFieldY").GetComponent<InputField>().text;
            //var zAxis = menu.transform.Find("Background").Find("TeleTab").Find("InputFieldZ").GetComponent<InputField>().text;
            //if (xAxis == string.Empty || yAxis == string.Empty || zAxis == string.Empty)
            //{
            //    HNotify.Get().AddNotification(HNotify.NotificationType.normal, "Coordinates cannot be empty", 3, HNotify.ErrorSprite);
            //    return;
            //}
            MainLevel.Instance.SetDayTime(9, 0);
        });

        menu.transform.Find("Background").Find("TeleTab").Find("InputFieldX").GetComponent<InputField>().contentType = InputField.ContentType.DecimalNumber;
        menu.transform.Find("Background").Find("TeleTab").Find("InputFieldY").GetComponent<InputField>().contentType = InputField.ContentType.DecimalNumber;
        menu.transform.Find("Background").Find("TeleTab").Find("InputFieldZ").GetComponent<InputField>().contentType = InputField.ContentType.DecimalNumber;
    ...
    public void Update()
    {
        if (GHAPI.IsCurrentSceneMainMenu())
        {
            if (check)
            {
                check = false;
        ...
        if (Input.GetKeyDown(KeyCode.F2))
        {
            ...
            if (!itemsInitialized && GHAPI.IsCurrentSceneGame())
            {
        ...
        if (GHAPI.IsCurrentSceneGame() && menuInitialized != null)
        {
            DebugMenuGH.GetAllDebug();
            DebugMenuGH.ShowDebugWound();
        }
        var playerInitialized = Player.Get();
        if (GHAPI.IsCurrentSceneGame() && playerInitialized != null)
        {
            if (flyenable)
            {
    ...
    public void OnModUnload()
    {
        ...
        Debug.Log("Mod GreenHell Menu has been unloaded!");
    }
}